      ...
      <div class="task-list">
        {#each tasks as task}
          <div class="task-item" class:done={task.done}>
            ...
            <button class="del-btn" onclick={() => removeTask(task.id)} aria-label={$t("common.remove")}>✕</button>
          </div>
        {/each}
        {#if tasks.length === 0}
      ...
      <div class="parked-list">
        {#each parked as thought, i}
          <div class="parked-item">
            <span>💭 {thought}</span>
            <button class="del-btn" onclick={() => removeParked(i)} aria-label={$t("common.remove")}>✕</button>
          </div>
        {/each}
        {#if parked.length === 0}
